...
class GEDTrainer(object):
    ...
    def fit(self):
        ...
        for iteration in range(self.max_iterations):
            ...
            np_train_true_pred = train_true_pred.cpu().detach().numpy()
            
            train_rho = metrics_spearmanr_rho(np_train_true_pred[:, 0], np_train_true_pred[:, 1])
            train_tau = metrics_kendall_tau(np_train_true_pred[:, 0], np_train_true_pred[:, 1])
            
            # print in training steps
            if iteration % int(self.max_iterations / 20) == 0:
                time_spent = datetime.now() - time
                time = datetime.now()
                write_log_file(self.log_file, "Iteration = {}\n\tbatch loss={} (e-3), rho = {}, tau={} @ {}".format(iteration, train_loss * 1000, train_rho, train_tau, time_spent))
    ...
    def testing(self):
        ...
        ps, inclusive_true_ks, inclusive_pred_ks = computing_precision_ks(trues=self.dataset.ground_truth, predictions=test_predictions, ks=[10, 20],
