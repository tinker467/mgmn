...
class GEDDataset(object):
    def __init__(self, ged_main_dir, args):
        ...
        data = simgnn_utils.SiameseDataSet(data_save_folder=os.path.join(self.ged_main_dir, 'data'), data_set_name=args.dataset, validation_ratio=0.25,
                                           node_feat_name=node_feat_name_dict[args.dataset], node_encoder_name=node_encoder_name_dict[args.dataset])
        ...
        true_result_test_train = simgnn_utils.load_result(dataset=data.data_set_name.lower(), model='astar', result_folder=os.path.join(self.ged_main_dir, 'result'), sim=None,
                                                          sim_mat=None, dist_mat=None, row_graphs=self.testing_graphs, col_graphs=self.train_val_graphs, scale=1.0)
    ...
    def extract_test_matrices(self, row):
        ...
        return np.array(batch_rows_feature), np.array(batch_rows_adjacent), np.array(batch_rows_mask), np.array(
            batch_cols_feature), np.array(batch_cols_adjacent), np.array(batch_cols_mask)
...
class CFGDataset(object):
    def __init__(self, data_dir, batch_size):
        ...
        if len(perm) < len(self.classes):
            perm = np.random.permutation(len(self.classes))
            np.save(class_perm_path, perm)
        graphs_train, classes_train, graphs_dev, classes_dev, graphs_test, classes_test = self.partition_graph_dataset(
            self.gs, self.classes, [0.8, 0.1, 0.1], perm)
